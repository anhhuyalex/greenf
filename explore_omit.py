...
# true_Y = OrderedDict()
pred_Y = OrderedDict()
successive_omits = OrderedDict()
# energy = np.random.uniform(-1, 1) 
tic = time.time()
omit = np.full(N**3, 1)
# drops = 0 # how many states have we dropped so far
epsilon = 7.03704e-7 # slope of line through (0, 0.0005) and (15000, 0.01)  -- how tolerant we are as drop increases
threshold = 0.01
for _ in range(N**3)[::-1]:
    ...
    # toc = time.time()
    # print ("Time building omit dicts", toc - tic)
    # omit = np.full(N**3, 1)
    # # for i in range(10):
    #     # omit[i] = 0
    # omit[_] = 0
